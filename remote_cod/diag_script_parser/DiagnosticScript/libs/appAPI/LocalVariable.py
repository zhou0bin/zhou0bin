...
class StatStep(LocalVariable):
    def __init__(self, ID,ECUName,EnglishTranslation,ChineseTranslation,PrintAtNok,PrintAtOK,StatBlock,BaseDataType,GUID,Value,VariableType,DefaultValue):
        self.ID = ID
...
def GeneratedParameterType(input_parameter):
     if input_parameter is None:
        ...
        for i in range(len(input_parameter)):
            if (ord(input_parameter[i]) >=32) and (ord(input_parameter[i]) <=126):
                strlist += input_parameter[i]
            else:
                strlist += "*"
