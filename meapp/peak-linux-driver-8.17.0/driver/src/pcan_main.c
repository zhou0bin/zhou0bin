...
{
	int sizeof_dev = sizeof(struct pcandev);
	struct pcandev *dev;
	void *p;

#ifndef NETDEV_SUPPORT
	if (rxqprealloc)
		sizeof_dev += rxqsize * sizeof(struct pcanfd_rxmsg);
#endif
	if (txqprealloc)
		sizeof_dev += txqsize * sizeof(struct pcanfd_txmsg);

	p = pcan_malloc(sizeof_dev, GFP_KERNEL);
	if (!p) {
		pr_err(DEVICE_NAME
	...
	if (rxqprealloc) {
		dev->rx_msgs = p;
		p += rxqsize * sizeof(struct pcanfd_rxmsg);
	}
#endif
	if (txqprealloc)
		dev->tx_msgs = p;
...
static int __init pcan_init_module(void)
{
	...
	/* give a qucik descr of some module parameters values */
#ifndef NETDEV_SUPPORT
