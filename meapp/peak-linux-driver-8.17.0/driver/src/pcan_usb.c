...
/* feature_flag */
#define PCAN_USB_VendorRequest_SDO_DOWNLOAD	0x00000001
#define PCAN_USB_VendorRequest_BUSPOWER		0x00000002

/* type */
#define PCAN_USB_VendorRequest_TYP_PEAK		0
#define PCAN_USB_VendorRequest_TYP_LENZE	1
#define PCAN_USB_VendorRequest_TYP_DC1		2
#define PCAN_USB_VendorRequest_TYP_DC1_HUB	3

static int pcan_usb_get_hwver(struct pcan_usb_interface *usb_if,
			      struct pcan_version *hw_ver)
{
	struct pcan_usb_vendor_request req_buffer;
	int err;
	...
	err = pcan_usbpro_request(usb_if, USB_VENDOR_REQUEST_INFO,
				  0, &req_buffer, sizeof(req_buffer));
	if (err)
		return err;

	hw_ver->major = req_buffer.version[0];
	hw_ver->minor = req_buffer.version[1];
	hw_ver->subminor = req_buffer.version[2];
...
int pcan_usb_init(struct pcan_usb_interface *usb_if)
{
	...
	if (usb_if->ucRevision >= 21) {
		int err = pcan_usb_get_hwver(usb_if, &usb_if->hw_ver);
		if (!err)
			dev_info(&usb_if->usb_intf->dev, "%s fw v%u.%u.%u\n",
